...
class TestExpenseTracker(unittest.TestCase):
    def setUp(self):
        ...
        # Smazání předchozích testovacích dat
        self.data_manager.save_data(self.test_user, {})
        # Uložení nových testovacích dat
    ...
    def test_duplicate_entry(self):
        ...
        original_data = self.data_manager.load_data(self.test_user)
        original_count = len(original_data["Nájem"])
        
        # Pokus o přidání duplicitního záznamu
        result = self.data_manager.add_entry(self.test_user, "Nájem", duplicate_entry)
        
        # Ověření, že nebyl přidán duplicitní záznam
        updated_data = self.data_manager.load_data(self.test_user)
        self.assertEqual(len(updated_data["Nájem"]), original_count)
        self.assertEqual(updated_data["Nájem"][0]["amount"], 15000.0)
        self.assertTrue(result)  # Mělo by vrátit True i když je záznam duplicitní
